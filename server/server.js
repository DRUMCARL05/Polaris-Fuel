...
async function getMarketStatus(
  ...
  try {
    ...
    if (resourceBalanceinVault && atlasBalanceInVault) {
      ...
      TradeData.resourceBalanceinVault = convertToBase10(
        resourceBalanceinVault,
        0
      );
    ...
    if (category.name === categoryName) {
      for (let asset of category.assets) {
        if (asset.name === assetName) {
          ...
          asset.beneficiary_resource_account =
            newInfo.beneficiary_resource_account;
          asset.beneficiary_percent = newInfo.beneficiary_percent;
          asset.resourceBalanceinVault =
            newInfo.resourceBalanceinVault.toString();
          asset.atlasBalanceInVault = newInfo.atlasBalanceInVault.toString();
          asset.soldOut =
            newInfo.resourceBalanceinVault < newInfo.minimum_buy_qty;
        }
      }
    }
  }
...
await getMarketStatus(
  new PublicKey(auth),
  new PublicKey(mints[pairIndex]),
  categoryAssetPairs[pairIndex].categoryName,
  categoryAssetPairs[pairIndex].assetName
);
pairIndex = (pairIndex + 1) % categoryAssetPairs.length;
